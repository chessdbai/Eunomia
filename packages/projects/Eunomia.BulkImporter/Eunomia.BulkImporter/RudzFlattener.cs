...
namespace Eunomia.BulkImporter
{
    ...
    {
        private readonly HashSet<string> movesHashSet = new HashSet<string>();

        /// <inheritdoc cref="IGameFlattener"/>
        public (IndexedGame, List<GamePositionRow>) FlattenPgnGame(
            PgnGame pgnGame,
            string pgnText,
            DatasetId datasetId)
        {
            var indexedGame = this.CreateGameFromPgnGame(pgnGame, datasetId, pgnText);
            ...
            foreach (var ply in pgnPlies)
            {
                ...
                if (!ply!.SanIsNullMove)
                {
                    ...
                    this.movesHashSet.Add(position.NextMoveSan);
                    this.movesHashSet.Add(position.NextMoveUci);

                    if (this.movesHashSet.Count % 100 == 0)
                    {
                        Console.WriteLine($"Moves hash set reached {this.movesHashSet.Count} moves.");
                    }

                    if (this.movesHashSet.Count >= 1000)
                    {
                        System.IO.File.WriteAllLines("/Users/john/moves.txt", this.movesHashSet);
                    }
        ...
        private IndexedGame CreateGameFromPgnGame(PgnGame game, DatasetId datasetId, string pgnText)
        {
            var indexedGame = new IndexedGame()
            {
                ...
                Event = game.AllTags["Round"].Value,
                Date = game.AllTags["Date"].AsDateValue() !.Value,
            ...
            indexedGame.Id = PgnIdUtility.GenerateGameId(game, pgnText);
